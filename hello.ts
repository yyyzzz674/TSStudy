...
myFavoriteNumber = 7
// -------------------------------------------------
let myFavoriteNumber1: string | number // 联合类型
myFavoriteNumber = 'seven'
myFavoriteNumber = 7
// -------------------------------------------------
...
interface Person {
  name: string
  age?: number // 可选属性
  // [propName: string]: any  // 可以为任意的属性值
  // [propName: string]: string  // 规定属性值只能为string类型,age属性值为number,也会报错
  [propName: string]: string | number
  readonly id: number // 只能在创建的时候被赋值
  // 只读的约束存在于第一次给对象赋值的时候，而不是第一次给只读属性赋值的时候, 即先创建对象再赋值也会报错
}
